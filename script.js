...
const relicPool = [
  ...
  { name: 'Reflexos', description: 'Aumenta em 45% a velocidade de movimento das duas raquetes.', image: 'assets/swift.svg', apply: () => { paddle.speed *= 1.45; relics.push({ name: 'Reflexos', icon: '➤' }); } },
...
function moveComputer() { const target = balls[0]?.y - paddle.height / 2; if (target == null) return; const computerSpeed = 4.6 * (hasRelic('Reflexos') ? 1.45 : 1); if (computer.y < target) computer.y += computerSpeed; if (computer.y > target) computer.y -= computerSpeed; clampPaddle(computer); }
...
function scorePoint(scoringPlayer) {
  ...
  // A recompensa acontece a cada 2 pontos somados no placar geral,
  // independentemente de quem marcou o ponto.
  if ((player.score + computer.score) % 2 === 0) {
    showChoices();
  } else {
...
function draw() {
  ...
  drawRect(player.x, player.y, paddle.width, paddle.height, '#67e8f9'); drawRect(computer.x, computer.y, paddle.width, paddle.height, '#f472b6');
...
function showChoices() {
  ...
  shuffledChoices().forEach((relic) => {
    ...
    card.querySelector('button').addEventListener('click', () => chooseRelic(relic)); cardsElement.appendChild(card);
  });
  overlay.hidden = false;
  cardsElement.querySelector('button')?.focus();
}
...
function resetGame() {
  player.score = 0; computer.score = 0; combo = 0; shields.player = 0; shields.computer = 0; relics.length = 0; paddle.height = 100; paddle.speed = 8; player.y = canvas.height / 2 - paddle.height / 2; computer.y = player.y; activeRelicsElement.innerHTML = ''; updateScoreboard(); overlay.hidden = true; paused = false; resetBalls();
}
